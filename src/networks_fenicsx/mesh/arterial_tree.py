...
Slightly adapted from Ingeborg Gjerde's adaptation of code
created by Alexandra Vallet (University of Oslo)
See https://github.com/IngeborgGjerde/graphnics/ and
https://gitlab.com/ValletAlexandra/NetworkGen for the originals.
    ...
    for igen in range(1, N):
        ...
        for e in previous_edges:
            ...
            for sign, angle, L, D in [branch1, branch2]:
                ...
                # If no edges overlap with this new one, just add it
                if True:  # FIXME!
                    new_node_ix += 1
...
def to_networks_graph(G_nx, cfg: config.Config):
    """
